...
public final class BiomeRulesManager extends SimplePreparableReloadListener<Map<ResourceLocation, BiomeRulesManager.BiomeRules>> {
    ...
    @Override
    protected Map<ResourceLocation, BiomeRules> prepare(ResourceManager manager, ProfilerFiller profiler) {
        ...
        for (Map.Entry<ResourceLocation, Resource> entry : resources.entrySet()) {
            try (InputStream stream = entry.getValue().open()) {
                ...
                LOGGER.error("Failed to load biome rules from {}: {}", entry.getKey(), e.getMessage());
            }
        }
    ...
    @Override
    protected void apply(Map<ResourceLocation, BiomeRules> rules, ResourceManager manager, ProfilerFiller profiler) {
        biomeRules.clear();
        biomeRules.putAll(rules);
    }

    private static BiomeRules parseBiomeRules(JsonObject biomeJson) {
        ...
        for (Map.Entry<String, JsonElement> ruleEntry : biomeJson.entrySet()) {
            ...
            if (type == null) {
                LOGGER.warn("Unknown rule type: {}", ruleEntry.getKey());
                continue;
            }
            try {
                rules.addRule(ruleEntry.getKey(), type.parse(ruleEntry.getValue()));
            } catch (Exception e) {
                LOGGER.warn("Invalid rule format for {}: {}", ruleEntry.getKey(), e.getMessage());
            }
        }
        return rules;
    }
